...
export class ArcadeManager {
  constructor() {
    ...
    this.popOutBtn = document.getElementById('arcadePopOutBtn');
  ...
  checkUrlParams() {
    try {
      ...
      if (isStandalone && gameKey && this.engines[gameKey]) {
        document.body.classList.add('standalone-arcade-mode');
        if (this.backToLobbyBtn) {
          this.backToLobbyBtn.textContent = '✕ Close Window';
        }
        this.launchCabinet(gameKey);
      } else if (gameKey && this.engines[gameKey]) {
        this.launchCabinet(gameKey);
      }
    } catch (err) {
      console.warn('URL parameter parsing failed:', err);
    }
  }

  openGameWindow(gameKey) {
    if (!gameKey || !this.engines[gameKey]) return;
    const url = `/?game=${encodeURIComponent(gameKey)}&standalone=1`;
    const winName = `TruckinArcade_${gameKey}`;
    const win = window.open(
      url,
      winName,
      'width=740,height=920,menubar=no,toolbar=no,location=no,status=no,resizable=yes'
    );
    if (!win || win.closed || typeof win.closed === 'undefined') {
      // Browser popup blocked - fall back to inline cabinet launch
      this.launchCabinet(gameKey);
    }
  }
  ...
  handleGameOver(score, hi) {
    ...
    if (this.overlayMessage) {
      this.overlayMessage.textContent = `Score: ${score} | Record: ${hi}. Tap screen or press Space to restart.`;
    }
    this.showOverlay();
  }
  ...
  returnToLobby() {
    if (document.body.classList.contains('standalone-arcade-mode')) {
      try {
        window.close();
      } catch (e) {}
      if (!window.closed) {
  ...
  bindEvents() {
    ...
    // Dedicated new window popup launch buttons
    document.querySelectorAll('[data-popup]').forEach((btn) => {
      btn.addEventListener('click', (e) => {
        e.stopPropagation();
        this.openGameWindow(btn.dataset.popup);
      });
    });
    ...
    if (this.popOutBtn) {
      this.popOutBtn.addEventListener('click', () => {
        if (this.activeKey) {
          this.openGameWindow(this.activeKey);
        }
      });
    }
    ...
    if (this.restartBtn) {
      ...
      this.restartBtn.addEventListener('pointerdown', doRestart);
    ...
    window.addEventListener('keydown', (e) => {
      ...
      if (this.activeEngine === this.clockworkEngine) {
        ...
        if (e.code === 'Space' || key === ' ') {
          // Space bar explicitly resets game and starts over immediately!
          this.hideOverlay();
